...
func _process(_delta):
	...
	if (goal_position-position).length() > 20:
		var dir := ((goal_position-position)/10).limit_length(5)
		position += dir
...
func choose_pos():
	...
	goal_position = Vector2(randf_range(0, DisplayServer.window_get_size().x-r.size.x), randf_range(0, DisplayServer.window_get_size().y-r.size.y))
